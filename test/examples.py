...
def test_lp(X, t):
    # f(X)
    # The original was (X[0]+X[1])->MAX
    # I reversed function's arguments signs and turn it into equivalent (-X[0]-X[1])->MIN
    func_x = - X[0] - X[1]

    # Gradient
    grad_x = np.array(np.zeros(2), dtype=np.float64)

    grad_x[0] = - t + 1/(1-X[0]-X[1]) - 1/(X[0]-2)
    grad_x[1] = - t + 1/(1-X[0]-X[1]) - 1/(X[1]-1) - 1/X[1]

    # Hessian
    func_h = (1 / (1-X[0]-X[1]))**2
    hessian_x = np.array(np.zeros((2, 2)), dtype=np.float64)
    hessian_x[0][0] = func_h + (1/(X[0]-2))**2
